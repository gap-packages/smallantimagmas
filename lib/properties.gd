...
#! @Arguments M
#! @Description
#!  identifies whether magma <A>M</A> is cyclic.
#!  An antimagma is cyclic if there exists an element $m \in M$ such that
#!  the submagma generated by $m$ equals $M$.
DeclareProperty("IsCyclic", IsMagma);
