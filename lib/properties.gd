...
#! @Arguments M
#! @Description
#! returns a generating set of the finite magma <A>M</A> of the least possible
#! size, the first such set in the order of <C>Combinations</C> of the elements
#! of <A>M</A>. Every antiassociative magma of order 2 or 3 is generated by one
#! element. For groups the methods of the &GAP; library apply instead.
#!
#! @BeginExampleSession
#! gap> List(AllSmallAntimagmas(3), MinimalGeneratingSet);
#! [ [ m3 ], [ m3 ], [ m1 ], [ m1 ], [ m1 ] ]
...
#! [ m3, m4 ]
#! gap> Submagma(M, MinimalGeneratingSet(M)) = M;
#! true
#! gap> ForAny(M, m -> Submagma(M, [m]) = M);
#! false
#! @EndExampleSession
#!
...
#! @Arguments M
#! @Description
#! returns the rank of the finite magma <A>M</A>, the least number of elements
#! that generate <A>M</A>, that is the size of
#! <Ref Attr="MinimalGeneratingSet" Label="for IsMagma"/>. This is a method for
#! the &GAP; operation <C>Rank</C>, which for groups keeps its meaning of the
#! rank of a <M>p</M>-group. Up to order 4 the rank of an antiassociative magma
#! is 1 or 2, and all but 183 of the isomorphism classes of order 4 have rank 1.
#!
#! @BeginExampleSession
#! gap> List(AllSmallAntimagmas(3), Rank);
#! [ 1, 1, 1, 1, 1 ]
#! gap> Rank(SmallAntimagma(4, 1));
#! 2
